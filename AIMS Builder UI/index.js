// console.log(steg.encode('Hello How Are You?' , 'data:image/gif;base64,'  ));

// console.log(steg.decode('data:image/png;base64,'));

var imgdatauri;

function readURL(input) {
//console.log("in read url");
  if (input.files && input.files[0]) {
    var reader = new FileReader();

    reader.onload = function(e) {
      document.querySelector("#image1").src = e.target.result;
      imgdatauri = e.target.result;
    };
  }
  reader.readAsDataURL(input.files[0]);
}


function decode(input) {
    if (input.files && input.files[0]) {
      var reader = new FileReader();
  
      reader.onload = function(e) {
          console.log("Decode")
          console.log(steg.decode(e.target.result));
        
        document.querySelector('#decoded').innerText = steg.decode(e.target.result);
      };
    }
    reader.readAsDataURL(input.files[0]);
  }


function hideText() {
  document.querySelector("#image2").src = steg.encode(document.querySelector('#result_text').innerHTML, 'http://localhost:5000/static/files/image.jpg');
//  document.querySelector("#image2").src = steg.encode("hardcoded", 'http://localhost:5000/static/files/image.jpg');

}

//if((document.querySelector('#result_text').innerHTML)!=null || (document.querySelector('#result_text').innerHTML)!='' )
//{
//  console.log("in if stmt");
//  hideText();
//}
